...
namespace RAMCloud {
struct ServerConfig {
    static constexpr uint32_t segletSize = 8 * 1024 * 1024;
    static constexpr uint32_t segmentSize = 8 * 1024 * 1024;

    struct Master {
        static constexpr double cleanerWriteCostThreshold = 0;
        static constexpr bool disableInMemoryCleaning = true;
        static constexpr int cleanerThreadCount = 1;
        static constexpr double diskExpansionFactor = 1.0;
        uint64_t logBytes;
    } master;
};
}
